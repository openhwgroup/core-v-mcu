...
CORE-V-MCU supports eight dedicated function top-level ports and a set of 40 user-selectable IO ports.
Dedicated functions are clock and reset plus JTAG and boot control functions.
The number of user-selectable IO ports is less than the total number of I/O signals supported by the CORE-V-MCU's internal peripheral functions,
so the **IO_CTRL** CSR (at offset 0x0400) is used to connect a subset of peripheral I/O to top-level CORE-V-MCU IO ports.
These selectable peripheral I/O are:
...
The table below provides the mapping between the above peripheral I/O and the IO ports of the CORE-V-MCU.
...
Please consult the _Digilent Nexys A7 FPGA Board Reference Manual_ for the locations and pinouts of the PMODs and other resources on the board.
The default mapping from CORE-V-MCU top-level IO to these Nexys A7 connections is provided in the table below.

| Nexys A7     | Func/IO   | Notes |
| ------------ | --------- | ----- |
| JB[10]       | jtag_tck  | |
| JB[8]        | jtag_tdi  | |
| JB[9]        | jtag_tdo  | |
...
| CLK100MHZ    | ref_clk   | Convinient on-board resource |
