...
ALLOWED_KEYS = [
    'incdirs', 'vlog_opts', 'vcom_opts', 'targets', 'flags', 'defines', 'dir',
    'sim_tools', 'synth_tools', 'jg_inclibs', 'jg_slint_top_name',
    'jg_slint_clocks', 'jg_slint_resets', 'jg_slint_elab_opt',
...
ALLOWED_TARGETS = [
    'all', 'rtl', 'verilator', 'xilinx', 'st28fdsoi', 'umc65', 'tsmc55',
    'tsmc40', 'gf28', 'gf22', 'smic130', 'smic110'
]

# list of allowed flags
ALLOWED_FLAGS = [
    'skip_simulation', 'skip_synthesis', 'skip_verilator', 'skip_tcsh',
    'only_local'
]
...
# list of allowed targets
ALLOWED_SIM_TOOLS = ['all', 'questa', 'xcelium', 'ncsim']

# list of allowed targets
ALLOWED_SYNTH_TOOLS = ['all', 'genus', 'dc', 'mentor']
...
class SubIPConfig(object):
    def __init__(self, ip_name, sub_ip_name, sub_ip_dic, ip_path):
        ...
        )  # target (all, xilinx, st28fdsoi, umc65, gf28 at the moment)
        self.flags = self.__get_flags(
        )  # flags (skip_simulation, skip_synthesis, skip_tcsh)
        ...
        self.sim_tools = self.__get_sim_tools(
        )  # eda tools supported (for RTL encrypted models)
        self.synth_tools = self.__get_synth_tools(
        )  # eda tools supported (for RTL encrypted models)
    ...
    def export_make(self,
        ...
        if len(vlog_includes) > 0:
            vlog_cmd += MK_SUBIPINC % (self.sub_ip_name,
                                       self.sub_ip_name.upper(),
                                       "+incdir" + vlog_includes)
        vlog_cmd += MK_SUBIPSRC % (self.sub_ip_name.upper(), vlog_files,
                                   self.sub_ip_name.upper(), vhdl_files)
        vlog_cmd += "\n"
        vlog_rule = ""
        if len(vlog_files) > 0:
            ...
            vlog_rule += mk_buildcmd_svlog % ("%s %s %s" %
                                              (more_opts, vlog_opts, defines),
                                              self.sub_ip_name.upper(),
                                              self.sub_ip_name.upper())
            vlog_rule += "\n"
            ...
            if len(vhdl_files) > 0:
                vlog_rule += "\t"
        if len(vhdl_files) > 0:
            vlog_rule += mk_buildcmd_vhdl % ("%s %s" % (more_opts, vcom_opts),
                                             self.sub_ip_name.upper())
            vlog_rule += "\n"
        vlog_cmd += mk_subiprule % (self.sub_ip_name, self.sub_ip_name,
                                    self.sub_ip_name, self.sub_ip_name.upper(),
                                    self.sub_ip_name.upper(), self.sub_ip_name,
                                    vlog_rule, self.sub_ip_name)
        vlog_cmd += "\n"

        return vlog_cmd

    def export_vsim(self,
                    abs_path,
                    more_opts,
                    target_tech='st28fdsoi',
                    local=False):
        if 'all' not in self.targets and 'rtl' not in self.targets and target_tech not in self.targets:
            return "\n"
        if target_tech == 'xilinx':
        ...
        for f in files:
            if not is_vhdl(f):
                vlog_cmd += VSIM_VLOG_CMD % (
                    "%s %s %s" % (more_opts, self.vlog_opts, defines),
                    vlog_includes, "%s/%s" % (abs_path, f))
            else:
                vlog_cmd += VSIM_VCOM_CMD % ("%s %s" %
                                             (more_opts, self.vcom_opts),
                                             "%s/%s" % (abs_path, f))
        return vlog_cmd
    ...
    def __export_vsim_xilinx(self, abs_path, more_opts):
        ...
        for f in files:
            if not is_vhdl(f):
                vlog_cmd += VSIM_VLOG_CMD % (
                    "%s %s %s" % (more_opts, vlog_opts, self.vlog_opts),
                    vlog_includes, "%s/%s" % (abs_path, f))
            else:
                vlog_cmd += VSIM_VCOM_CMD % ("%s %s" %
                                             (more_opts, self.vcom_opts),
    ...
    def export_synopsys(self, path, target_tech=None, source='ips'):
        if not ("all" in self.targets or target_tech is None
                or target_tech in self.targets):
            return "\n"
        ...
        for f in files:
            if is_vhdl(f):
                analyze_cmd += SYNOPSYS_ANALYZE_VHDL_CMD % (
                    self.sub_ip_name, source.upper(), "%s/%s" % (path, f))
            elif is_verilog_2001(f):
                analyze_cmd += SYNOPSYS_ANALYZE_V_CMD % (
                    defines, source.upper(), "%s/%s" % (path, f))
            else:
                analyze_cmd += SYNOPSYS_ANALYZE_SV_CMD % (
                    defines, source.upper(), "%s/%s" % (path, f))
        return analyze_cmd
    ...
    def export_cadence(self, path, target_tech='st28fdsoi', source='ips'):
        ...
        for f in files:
            if not is_vhdl(f):
                analyze_cmd += CADENCE_ANALYZE_SV_CMD % (
                    defines, source.upper(), "%s/%s" % (path, f))
            else:
                analyze_cmd += CADENCE_ANALYZE_VHDL_CMD % (
                    self.sub_ip_name, source.upper(), "%s/%s" % (path, f))
        return analyze_cmd
    ...
    def export_verilator(self, abs_path):
        ...
        if "skip_verilator" in self.flags:
            return "\n"
        verilator_mk = VERILATOR_PREAMBLE_SUBIP % (
            self.sub_ip_name, prepare(self.sub_ip_name.upper()))
        files = self.files
        for f in files:
            verilator_mk += "    %s/%s/%s \\\n" % (abs_path, self.ip_path, f)
        verilator_mk += VERILATOR_POSTAMBLE_SUBIP
        if len(self.incdirs) > 0:
            verilator_mk += VERILATOR_PREAMBLE_SUBIP_INCDIRS % prepare(
                self.sub_ip_name.upper())
            for i in self.incdirs:
    ...
    def export_vivado(self, abs_path):
        ...
        if "skip_synthesis" in self.flags:
            return "\n"
        vivado_cmd = VIVADO_PREAMBLE_SUBIP % (
            self.sub_ip_name, prepare(self.sub_ip_name.upper()))
        files = self.files
        for f in files:
            vivado_cmd += "    %s/%s/%s \\\n" % (abs_path, self.ip_path, f)
        ...
        if len(self.incdirs) > 0:
            vivado_cmd += VIVADO_PREAMBLE_SUBIP_INCDIRS % prepare(
                self.sub_ip_name.upper())
            for i in self.incdirs:
                vivado_cmd += "    %s/%s/%s \\\n" % (abs_path, self.ip_path, i)
            vivado_cmd += VIVADO_POSTAMBLE_SUBIP
    ...
    def __check_dic(self):
        ...
        if set(MANDATORY_KEYS).intersection(set(dic.keys())) == set([]):
            print(
                "ERROR: there are no files for ip '%s', sub-ip '%s'. Check its src_files.yml file."
                % (self.ip_name, self.sub_ip_name))
            sys.exit(1)
        not_allowed = set(dic.keys()) - set(MANDATORY_KEYS) - set(ALLOWED_KEYS)
        if not_allowed != set([]):
            print("ERROR: there are unallowed keys for ip '%s', sub-ip '%s':" %
                  (self.ip_name, self.sub_ip_name))
    ...
    def __get_flags(self):
        ...
        if not_allowed != set([]):
            print("ERROR: flags not allowed for ip '%s', sub-ip '%s':" %
                  (self.ip_name, self.sub_ip_name))
            print(not_allowed)
            for el in list(not_allowed):
                print("    %s" % el)
    ...
    def __get_targets(self):
        ...
        if not_allowed != set([]):
            print("ERROR: targets not allowed for ip '%s', sub-ip '%s':" %
                  (self.ip_name, self.sub_ip_name))
            print(not_allowed)
            for el in list(not_allowed):
                print("    %s" % el)
    ...
    def __get_sim_tools(self):
        ...
        if not_allowed != set([]):
            print("ERROR: sim_tools not allowed for ip '%s', sub-ip '%s':" %
                  (self.ip_name, self.sub_ip_name))
            print(not_allowed)
            for el in list(not_allowed):
                print("    %s" % el)
    ...
    def __get_synth_tools(self):
        ...
        if not_allowed != set([]):
            print("ERROR: synth_tools not allowed for ip '%s', sub-ip '%s':" %
                  (self.ip_name, self.sub_ip_name))
            print(not_allowed)
            for el in list(not_allowed):
                print("    %s" % el)
