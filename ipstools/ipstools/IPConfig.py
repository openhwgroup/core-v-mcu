...
class IPConfig(object):
    ...
    def export_make(self,
        ...
        for s in self.sub_ips.keys():
            if ("all" in self.sub_ips[s].targets
                    or "rtl" in self.sub_ips[s].targets
                    or target_tech in self.sub_ips[s].targets):
                if ("skip_simulation" not in self.sub_ips[s].flags and
                    (("only_local" not in self.sub_ips[s].flags) or local)):
                    if simulator == 'vsim':
                        if 'all' in self.sub_ips[
                                s].sim_tools or 'questa' in self.sub_ips[
                                    s].sim_tools:
                            commands += "$(LIB_PATH)/%s.%s " % (s, vmake)
                            phony += "vcompile-subip-%s " % s
                    elif simulator == 'ncsim':
                        if 'all' in self.sub_ips[
                                s].sim_tools or 'xcelium' in self.sub_ips[
                                    s].sim_tools or 'ncsim' in self.sub_ips[
                                        s].sim_tools:
                            commands += "$(LIB_PATH)/%s.%s " % (s, vmake)
                            phony += "ncompile-subip-%s " % s
        if self.ip_path[0] == '/':
            makefile = mk_preamble % (prepare(
                self.ip_name), '', self.ip_path[1:], phony, commands)
        else:
            makefile = mk_preamble % (prepare(
                self.ip_name), ip_path_env, self.ip_path, phony, commands)
        makefile += MK_POSTAMBLE
        for s in self.sub_ips.keys():
            makefile += self.sub_ips[s].export_make(abs_path,
                                                    more_opts,
    ...
    def export_vsim(self,
                    ...
                    local=False):
        vsim_script = VSIM_PREAMBLE % (self.vsim_dir, prepare(
            self.ip_name), self.ip_path)
        for s in self.sub_ips.keys():
    ...
    def export_synopsys(self, target_tech=None, source='ips'):
        ...
        for s in self.sub_ips.keys():
            analyze_script += self.sub_ips[s].export_synopsys(
                self.ip_path, target_tech=target_tech, source=source)
    ...
    def export_cadence(self, target_tech='st28fdsoi', source='ips'):
        ...
        for s in self.sub_ips.keys():
            analyze_script += self.sub_ips[s].export_cadence(
                self.ip_path, target_tech=target_tech, source=source)
        return analyze_script
    ...
    def generate_vivado_add_files(self):
        ...
        for s in self.sub_ips.keys():
            if (("xilinx" in self.sub_ips[s].targets
                 or "all" in self.sub_ips[s].targets)
                    and ("skip_synthesis" not in self.sub_ips[s].flags)):
                l.append(prepare(s))
        return l
    ...
    def generate_verilator_src(self):
        ...
        for s in self.sub_ips.keys():
            if (("all" in self.sub_ips[s].targets
                 or "verilator" in self.sub_ips[s].targets)
                    and ("skip_verilator" not in self.sub_ips[s].flags)):
                l.append(prepare(s))
        return l
