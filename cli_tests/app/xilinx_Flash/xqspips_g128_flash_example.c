...
unsigned char gCheatAddressSpace[256] = {0};
...
int x_main(void)
{
	...
	dbg_str("QSPI Greater than 128Mb Flash Example Test \r\n");
	...
	if (Status != XST_SUCCESS) {
		dbg_str("QSPI Greater than 128Mb Flash Example Test Failed\r\n");
		return XST_FAILURE;
	}

	dbg_str("Successfully ran QSPI Greater than 128Mb Flash Ex Test\r\n");
	return XST_SUCCESS;
}
...
int FlashReadID(XQspiPs *QspiPtr, u8 *WriteBfrPtr, u8 *ReadBfrPtr)
{
	...
	CLI_printf("FlashID=0x%x 0x%x 0x%x\n", ReadBfrPtr[1], ReadBfrPtr[2],
		   ReadBfrPtr[3]);
